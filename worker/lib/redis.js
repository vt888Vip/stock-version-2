...
class RedisManager {
    ...
    async atomicUpdateUserBalance(userId, changes, operation) {
    ...
    async atomicProcessTrade(tradeId, operation) {
    ...
    async updateBalance(userId, changes) {
        const client = this.getClient();
        const balanceKey = `user:${userId}:balance`;
        try {
    ...
    async isTradeProcessed(tradeId) {
        const client = this.getClient();
    ...
    async markTradeProcessed(tradeId, ttl = 3600) {
        const client = this.getClient();
        const tradeKey = `trade:${tradeId}:processed`;
        try {
    ...
    async setSessionResult(sessionId, result, ttl = 7200) {
        const client = this.getClient();
        const sessionKey = `session:${sessionId}:result`;
        try {
    ...
    async atomicUpdateSessionStats(sessionId, stats, operation) {
    ...
    async acquireLockWithRetry(key, ttl = 30000, maxRetries = 5, retryDelay = 100) {
