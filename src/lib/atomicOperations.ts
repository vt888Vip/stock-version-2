...
/**
 * Atomic Operations với Redis Locks để giải quyết Race Conditions
 */
 ...
 */
export async function atomicPlaceTrade(
  userId: string,
  sessionId: string,
  amount: number,
  type: 'buy' | 'sell'
  ...
  return await redisManager.atomicUpdateUserBalance(
    userId,
 ...
 */
export async function atomicCheckTradeResult(
  tradeId: string,
  userId: string,
  sessionId: string,
  amount: number,
    ...
    try {
      return await session.withTransaction(async () => {
        ...
        await redisManager.atomicUpdateSessionStats(sessionId, {
          ...
          return await db.collection('trading_sessions').updateOne(
            { sessionId },
            {
              $inc: {
                totalTrades: 1,
 ...
 */
export async function atomicSyncUserBalance(userId: string): Promise<AtomicOperationResult> {
 ...
 */
export async function atomicBatchProcessTrades(
  trades: Array<{ tradeId: string; userId: string; sessionId: string; amount: number; type: 'buy' | 'sell' }>
): Promise<AtomicOperationResult[]> {
  ...
  const promises = trades.map(async (trade) => {
    try {
      const result = await atomicCheckTradeResult(
        trade.tradeId,
        trade.userId,
        trade.sessionId,
        trade.amount,
 ...
 */
export async function atomicOperationsHealthCheck(): Promise<{
  redis: { healthy: boolean; latency?: number; error?: string };
  mongodb: { healthy: boolean; error?: string };
}> {
